...
// Relies on tb_pkg.sv being compiled first (see Makefile / tb_pkg.sv header).
...
module tb_scrambler;
    ...
    // Fixed-depth circular scoreboard (avoids SystemVerilog queue types per
    // doc 05's "avoid dynamic types where avoidable" guidance) — 2-cycle
    // pipeline (scrambler + descrambler each register once) never needs more
    // than a couple of entries in flight, depth 8 is ample headroom.
    localparam int HIST_DEPTH = 8;
    logic [31:0] hist_data [0:HIST_DEPTH-1];
    logic [3:0]  hist_ctrl [0:HIST_DEPTH-1];
    ...
    initial begin
        ...
        s_ctrl_i  <= 4'b1111; // all 4 octets marked K
        @(posedge clk);
        s_valid_i <= 1'b0;
        #1;
        `CHECK(s_valid_o, "scrambler valid_o expected after K-octet send")
        `CHECK(s_data_o === 32'hDEADBEEF, "K-marked octets must pass through scrambler unscrambled")
        @(posedge clk);
        #1;
        `CHECK(d_valid_o, "descrambler valid_o expected after K-octet send")
        `CHECK(d_data_o === 32'hDEADBEEF, "K-marked octets must round-trip unchanged through descrambler")
        ...
        // vectors, back-to-back valid_i every cycle (self-sync: no reset,
        // no gaps between vectors) ---
        for (i = 0; i < 4000; i = i + 1) begin
            rnd_data = $urandom;
            rnd_ctrl = { $urandom_range(0, 7) == 0, $urandom_range(0, 7) == 0,
                         $urandom_range(0, 7) == 0, $urandom_range(0, 7) == 0 };
            s_data_i  <= rnd_data;
            s_ctrl_i  <= rnd_ctrl;
            s_valid_i <= 1'b1;
