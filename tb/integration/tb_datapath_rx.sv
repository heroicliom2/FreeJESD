...
// Doc 06 exit criterion: passes for both SCR=0 and SCR=1.
//
// Also swept across DW_OCTETS in {2,4,8} (16/32/64-bit datapath widths,
// this project's width-flexibility requirement — see scrambler.sv's
// header), so all 6 (SCR x width) combinations run in one pass via nested
// generate rather than six separate Makefile targets. F_CFG=8 here (not
// Milestone 2's F=4) specifically so it's a multiple of all three widths
// (2, 4, and 8) — the project convention established for this requirement
// (F must be a multiple of DW_OCTETS, see docs/HANDOFF.md).
...
module tb_datapath_rx;
    ...
    localparam int L_CFG = 1, F_CFG = 8, K_CFG = 32, M_CFG = 2, N_CFG = 16, NP_CFG = 16, S_CFG = 1;
    ...
    generate
        for (gscr = 0; gscr < 2; gscr = gscr + 1) begin : g_scr
            ...
            for (gdw = 0; gdw < 3; gdw = gdw + 1) begin : g_dw
                ...
                logic       descr_valid_o;
                logic [DW*8-1:0] descr_data_o;
                logic [DW-1:0]   descr_ctrl_o;
                ...
                // hold a full multiframe (F*K/DW_OCTETS words), not the skew-sized
                // depth doc 03 actually specifies elastic_buffer.sv's DEPTH for.
                // For this single-lane milestone there's no other lane to deskew
                // against, so release continuously once ready — matches
                // ELASTIC_DEPTH=8.
                logic lmfc_zero;
                ...
                // produces a new word every DW_OCTETS octets, so most cycles
                // the buffer would be empty. Gating on level_o > 0 naturally
                // settles to the write rate without ever underflowing.
    ...
    initial begin
        wait (g_scr[0].g_dw[0].done && g_scr[0].g_dw[1].done && g_scr[0].g_dw[2].done &&
              g_scr[1].g_dw[0].done && g_scr[1].g_dw[1].done && g_scr[1].g_dw[2].done);
